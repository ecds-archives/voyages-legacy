...
public class DbExportTest extends TestCase{
	...
	private TastDbQuery getQuery() {
		...
		for (Iterator iterator = arrList.iterator(); iterator.hasNext();) {
			...
			query.addPopulatedAttribute(Voyage.getAttribute(attr));
		}
		...
		for (int i=0; i < rslt.length; i++) {			
			...
			if (voy != null) {
				...
				for (int j=0; j < voyages.size(); j++){
					...
					System.out.println("voyg shipname:" + voyg.getShipname());
					System.out.println("voyg voyageid:" + voyg.getVoyageid());
					voyMap.put(voyg.getIid(), name);
				}				
			}			
			
		}
		System.out.println("voyage map: " + voyMap.toString());
		//find all voyage ids associated with a submission
		
		/*TastDbQuery q = new TastDbQuery(new String("SubmissionEdit"));
		session = HibernateConn.getSession();
		tran = session.beginTransaction();
		Object[] rslt = q.executeQuery(session);
		for (int i=0; i < rslt.length; i++) {
			Submission sub = (Submission)rslt[i];
			String name = ((SubmissionEdit) sub) .getUser().getUserName();
			System.out.println("user name:" + name);
			EditedVoyage newVoy= ((SubmissionEdit) sub).getNewVoyage();
			System.out.println("new voyage iid:" + newVoy.getVoyage().getIid());;
			EditedVoyage voy= ((SubmissionEdit) sub).getEditorVoyage();
			System.out.println("edit voyage iid:" + voy.getVoyage().getIid());
			EditedVoyage oldVoy= ((SubmissionEdit) sub).getOldVoyage();
			System.out.println("old voyage iid:" + oldVoy.getVoyage().getIid());		
