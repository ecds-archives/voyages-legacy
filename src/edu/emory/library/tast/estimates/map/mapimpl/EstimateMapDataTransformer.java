	...
	public TransformerResponse transformData(
		...
		for (int i = 0; i < data.length; i++) {
			...
			if (exp != null) {
				...
				double numberExp = Math.round(((Number) row[1]).doubleValue() * 100) / (double)100;
...
//				}
		...
		for (Iterator iter = mapDataItems.iterator(); iter.hasNext(); i++) {
			...
			element.addLegendItem(legendSizes.getItems()[element.getSize() - 1]);
